...
/*		if (x < cx - dpadDeadZone)
		...
		if (y < cy - dpadDeadZone)
	      ...
	      if (x < cx - dpadDeadZone)
	            states |= KeyEvent.KEYCODE_DPAD_LEFT;
	        else if (x > cx + dpadDeadZone)
	            states |= KeyEvent.KEYCODE_DPAD_RIGHT;
	        if (y < cy - dpadDeadZone)
	            states |= KeyEvent.KEYCODE_DPAD_UP;
	        else if (y > cy + dpadDeadZone)
	            states |= KeyEvent.KEYCODE_DPAD_DOWN;
	...
	private int getButtonsStates(float x, float y, float size) {
		...
		if (size > pointSizeThreshold) {
			...
			case KeyEvent.KEYCODE_BUTTON_Y:
				states = (KeyEvent.KEYCODE_BUTTON_B | KeyEvent.KEYCODE_BUTTON_Y);
				break;
			case KeyEvent.KEYCODE_BUTTON_X:
			case KeyEvent.KEYCODE_BUTTON_A:
				states = (KeyEvent.KEYCODE_BUTTON_X |  KeyEvent.KEYCODE_BUTTON_A);
				break;
			}
		}
		return states;
	...
	private int getSelectStartStates(float x, float y) {
		return (x < 0.5f ?  KeyEvent.KEYCODE_BUTTON_SELECT : KeyEvent.KEYCODE_BUTTON_START);
	}
